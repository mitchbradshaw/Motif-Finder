...
### 2.12 The final gate, and one leak in the smoke itself
Type-check and production build clean. pytest at HEAD in the copied-DATA worktree: 39 failed / 1298 passed,
the failure set identical line for line to the Task 1 gate (`webui/PYTEST_GATE_FINAL.txt`); no file pytest collects
changed after that gate. The real `DATA/` is unchanged from the start snapshot: every file under `DATA/derived/` and
both database files keep their mtime and size (12,125 entries compared). The first full smoke against the production
bridge (live flows, then every page state) reported 577 states and two failures, both caused by the smoke, not the
pages: the loud-failure flow deliberately trips a chain row's error boundary with `?throw=1`, and the send-span flow
leaves a selection in the in-memory store, and the page walk then began in the same tab without a reload, so the
first Analyse state inherited the red card and Discovery's "no span selected in Explore" state found a span. The
dev-server run (`--pages-only`, no flows first) had been green for the same reason. The walk now starts from a hard
reload; the states themselves keep sharing the tab, as every builder ran them. The rerun's line is recorded below.
The rerun still failed the Discovery state: the app keeps the source span in `sessionStorage` on purpose
(`state.tsx`: a reload keeps the source and the chain), so a reload is not a fresh tab. The walk now clears both
storages before its reload — the fresh-tab condition every `--pages-only` run had implicitly. **Final full smoke
against the production bridge: live flows plus 576 page states, 0 failures, 0 console errors, 0 unexpected server
tracebacks.** Screenshots of every state are committed under `webui/screenshots/pages/<unit>/`.
