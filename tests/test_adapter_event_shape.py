...
def test_defaults_and_bounds():
    ...
    assert p["polarity"] == "auto" and p["recovery_frac"] == 0.5 and p["knee_frac"] == 0.05
...
def test_a_spike_is_the_same_measurement_about_the_opposite_sign():
    ...
    s = _run(-drop()).value.features.iloc[0]
    assert s["polarity"] == 1
...
def test_forcing_polarity_overrides_auto():
    f = _run(drop(), polarity="drop").value.features.iloc[0]
    assert f["polarity"] == -1
    g = _run(-drop(), polarity="spike").value.features.iloc[0]
    assert g["polarity"] == 1
...
def test_the_rose_is_gradients_rose_data_over_the_events():
    meta = _run(np.concatenate([drop(), drop(), -drop()]), spans=((0, 300), (300, 600), (600, 900))).meta
    rose = meta["rose"]
